...
avg_data['opponent_id'] = (avg_data['bout_id'] + avg_data['fighter_id']).apply(lambda x: opponents[x] if x in opponents.keys() else np.nan)
avg_data.dropna(inplace = True)

avg_data = pd.merge(avg_data, stats[['bout_id', 'fighter_id', 'fight_date']], left_on = ['fighter_id', 'bout_id'], right_on = ['fighter_id', 'bout_id'])
stat_adj = {}
for fighter in avg_data.fighter_id.unique():
#    f_stats = {} 
    
    fstat = avg_data.loc[avg_data['fighter_id'] == fighter]
    fstat.sort_values('fight_date', inplace = True)

    ostat = avg_data.loc[avg_data['opponent_id'] == fighter]
    ostat.sort_values('fight_date', inplace = True) 
    
    if len(fstat) != len(ostat):
        raise ValueError()
        
    if (fstat[['bout_id', 'fight_date']].values != ostat[['bout_id', 'fight_date']].values).any():
        raise ValueError()    
