...
def store_model_cv_preds(domain):
    print('Validation set model CV predictions')
    X, Y = pull_eval_data(domain)
    ...
    pred_df.to_csv(os.path.join(cur_path, 'data', 'cv_predictions', 'raw_preds.csv'))   
#    from sklearn.linear_model import LogisticRegression
#    log_model = LogisticRegression(random_state = 1108)
#    log_model.fit(pred_df[[i for i in list(pred_df) if i != 'winner']], pred_df['winner'])
#    log_model.coef_ 
    ...
    vegas_pred_df.to_csv(os.path.join(cur_path, 'data', 'cv_predictions', 'vegas_preds.csv'))
