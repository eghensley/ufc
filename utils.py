...
#    _model, x, y, scale, start_score, verbose = lgb_clf, X[features], Y, scale, lgb_checkpoint_score, True
    ...
    return(model.set_params(**{'clf__learning_rate':param_search[max(param_search.keys())]['lr'], 'n_estimators': param_search[max(param_search.keys())]['best_trees']}), param_search[max(param_search.keys())]['best_score'])
