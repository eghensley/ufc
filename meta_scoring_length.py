...
pred_data_winner = pd.read_csv(os.path.join(cur_path, 'test_data', 'pred_data_length.csv'))
...
X = pred_data_winner[[i for i in list(pred_data_winner) if i != 'length']]
Y = pred_data_winner['length'].apply(lambda x: x if x == 1 else 0)
